...
function chooseList() {
   ...
   location.href = "./categories.do?genre=" +ckEvtTxt;
}
...
/*window.onload함수가 실행될때 url값 중에서 파라미터 값만 js로 추출하기(location.href사용)
location.href를 이용하면 현재 페이지 전체 URL을 가져올 수 있다.
*/
   let url = new URL(window.location.href);
   let urlParam = url.searchParams;
   let searchGenre = urlParam.get('genre');
   console.log(searchGenre);	//장르 이름이 나온다
   
   if(searchGenre == null || searchGenre == ""){
      loadDefaultList();
   }else{
      loadSearchGenreList(searchGenre);
   }
...
function loadSearchGenreList(searchGenre){
   ckEvtTxt = searchGenre;
   //영화정보url
   let mvUrl = 'https://api.themoviedb.org/3/movie/popular?api_key=b96ed5ac7fac8cd9c2670c50e891b392&language=ko-KR&page=1';
   
   let genreMovieList = new Array();
               ...
               for (j = 0; j < mvList.length; j++) {
                  for (i = 0; i < grList.length; i++) {
                     if (ckEvtTxt == grList[i].name) {   //목록의 장르이름이랑 장르url의 장르 이름이랑 같을때
                        ...
                        if (gerNum != -1) {   //포함되어 있을때만 
                           console.log(mvList[j])   //해당 장르번호의 영화정보만..콘솔은 되는데..화면에나오게 어케함?
//                           posterPath = mvList[j].poster_path;
//                           movieTitle = mvList[j].original_title;
                           
                           makeMovieList(mvList[j]);
                        }
                     }
                  }
               }
...
function makeMovieList(movieData){
   ...
   let poster = row.children[resultCnt].children[0].children[0];
   
   let imgUrl = 'https://image.tmdb.org/t/p/';
   let imgUrlSize = "w342";

   poster.setAttribute('style', 'background-image: url("' + imgUrl + imgUrlSize + movieData.poster_path + '"); background-size: 230px 325px;')
   ...
   //제목 부분
   let mvtitle = row.children[resultCnt].children[0].children[1].children[1].children[0];

   mvtitle.innerText = movieData.title;

   //제목부분 링크 수정
   //let txtTag = document.querySelector('div .product__item__text h5').children[0]
   txtTag = row.children[resultCnt].children[0].children[1].children[1].children[0]
